...
const GuruExamPage: React.FC = () => {
  ...
  // Load daftar siswa on component mount
  useEffect(() => {
    loadDaftarSiswa()
  }, [])

  // Load exam data when selectedSiswa changes
  useEffect(() => {
    if (selectedSiswa) {
      loadExamData()
    }
  ...
  const loadExamData = async () => {
    ...
    try {
      ...
      const response = await guruAPI.getJadwalUjian(selectedSiswa.siswa_id, tanggal_mulai, tanggal_selesai)
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('401') || error.message.includes('tidak berhak') || error.message.includes('Unauthorized')) {
          setError('Anda tidak memiliki akses untuk melihat jadwal ujian siswa ini. Pastikan Anda login sebagai guru yang benar.')
        } else if (error.message.includes('403') || error.message.includes('Forbidden')) {
          setError('Akses ditolak. Anda mungkin tidak memiliki izin untuk melihat data ini.')
        } else if (error.message.includes('404')) {
          setError('Data jadwal ujian tidak ditemukan untuk siswa ini.')
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{
              ...
              Kelola dan pantau jadwal ujian siswa dalam kelas Anda
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        >
          ...
          <div style={{
            ...
            {daftarSiswa.map((siswa) => (
              <motion.div
                ...
                style={{
                  background: selectedSiswa?.siswa_id === siswa.siswa_id 
                    ? COLORS.primary 
                    : COLORS.white,
                  color: selectedSiswa?.siswa_id === siswa.siswa_id ? COLORS.white : COLORS.primary,
                  border: selectedSiswa?.siswa_id === siswa.siswa_id 
                    ? 'none' 
                  ...
                  boxShadow: selectedSiswa?.siswa_id === siswa.siswa_id 
              ...
              >
                ...
                <div style={{
                  ...
                  {siswa.nis} - Kelas {siswa.nama_kelas}
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div style={{ position: 'relative' }}>
            <select
              value={selectedSiswa?.siswa_id || ''}
              onChange={(e) => {
                const siswa = daftarSiswa.find(s => s.siswa_id === parseInt(e.target.value))
            ...
            >
              ...
              {daftarSiswa.map((siswa) => (
                <option key={siswa.siswa_id} value={siswa.siswa_id}>
                  {siswa.nama_lengkap} - {siswa.nis} - Kelas {siswa.nama_kelas}
                </option>
              ))}
            </select>
            <ChevronDown 
        ...
        {!isLoadingSiswa && daftarSiswa.length === 0 && (
          ...
          >
            ...
            <p style={{ margin: '0 0 15px 0' }}>
              Tidak dapat memuat daftar siswa. Pastikan Anda login sebagai guru dan memiliki akses ke data siswa.
            </p>
            <button
        ...
        {!selectedSiswa && daftarSiswa.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            style={{
          ...
          >
            <GraduationCap size={48} color="#cbd5e1" style={{ margin: '0 auto 20px' }} />
            <h3 style={{ fontSize: '18px', fontWeight: '600', margin: '0 0 10px 0' }}>
              Pilih Siswa Terlebih Dahulu
            </h3>
            <p style={{ margin: 0 }}>
              Silakan pilih siswa dari daftar di atas untuk melihat jadwal ujian
            </p>
          </motion.div>
        )}
            ...
            >
              <div style={{
                ...
                <div style={{
                  ...
                  <Filter size={18} />
                  Jadwal Ujian untuk {selectedSiswa.nama_lengkap}
                </div>
                <button
            ...
            {error ? (
              ...
              >
                ...
                <p style={{ margin: 0 }}>
                  ...
                  } untuk {selectedSiswa.nama_lengkap}
                </p>
              </motion.div>
            )}
          </>
